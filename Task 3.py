...
class GUI():
    def __init__(self, master):
        ...
        root.config(bg='#FFD700')  # Change to a gold background
        root.resizable(False, False)

        self.label = Label(text=":PASSWORD GENERATOR:", anchor=N, fg='navy', bg='#FFD700', font='arial 20 bold underline')
        self.label.grid(row=0, column=1)

        self.blank_label1 = Label(text="", bg='#FFD700')
        self.blank_label1.grid(row=1, column=0, columnspan=2)
        
        self.blank_label2 = Label(text="", bg='#FFD700')
        self.blank_label2.grid(row=2, column=0, columnspan=2)    

        self.blank_label3 = Label(text="", bg='#FFD700')
        self.blank_label3.grid(row=3, column=0, columnspan=2)    

        self.user = Label(text="Enter User Name: ", font='times 15 bold', bg='#FFD700', fg='navy')
        self.user.grid(row=4, column=0)

        self.textfield = Entry(textvariable=self.n_username, font='times 15', bd=6, relief='ridge', fg='black')
        self.textfield.grid(row=4, column=1)
        self.textfield.focus_set()

        self.blank_label4 = Label(text="", bg='#FFD700')
        self.blank_label4.grid(row=5, column=0)

        self.length = Label(text="Enter Password Length: ", font='times 15 bold', bg='#FFD700', fg='navy')
        self.length.grid(row=6, column=0)

        self.length_textfield = Entry(textvariable=self.n_passwordlen, font='times 15', bd=6, relief='ridge', fg='black')
        self.length_textfield.grid(row=6, column=1)
        
        self.blank_label5 = Label(text="", bg='#FFD700')
        self.blank_label5.grid(row=7, column=0)
 
        self.generated_password = Label(text="Generated Password: ", font='times 15 bold', bg='#FFD700', fg='navy')
        ...
        self.blank_label6 = Label(text="", bg='#FFD700')
        self.blank_label6.grid(row=9, column=0)

        self.generate = Button(text="GENERATE PASSWORD", bd=3, relief='solid', padx=1, pady=1, font='Verdana 15 bold', fg='white', bg='#32CD32', command=self.generate_pass)
        self.generate.grid(row=11, column=1)

        self.blank_label7 = Label(text="", bg='#FFD700')
        self.blank_label7.grid(row=12, column=0)

        self.accept = Button(text="ACCEPT", bd=3, relief='solid', padx=1, pady=1, font='Helvetica 15 bold italic', fg='white', bg='#4682B4', command=self.accept_fields)
        self.accept.grid(row=13, column=1)

        self.blank_label8 = Label(text="", bg='#FFD700')
        self.blank_label8.grid(row=14, column=1)

        self.reset = Button(text="RESET", bd=3, relief='solid', padx=1, pady=1, font='Helvetica 15 bold italic', fg='white', bg='#FF6347', command=self.reset_fields)
